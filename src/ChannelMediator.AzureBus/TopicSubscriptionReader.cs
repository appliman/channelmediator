...
internal sealed class TopicSubscriptionReader : IAsyncDisposable
{
	...
	{
		await _entityManager.EnsureTopicExistsAsync(_options.TopicName, cancellationToken);
	...
	public async Task StartAsync(CancellationToken cancellationToken)
	{
		...
		if (!_initialized)
		{
			await EnsureTopicAndSubscriptionExistAsync(cancellationToken);
			_initialized = true;
		}
	...
	private async Task ProcessMessageAsync(ProcessMessageEventArgs args)
	{
		try
		{
			...
			await DispatchNotificationAsync(notification, args.CancellationToken);
		}
		catch (Exception ex)
		{
